...
## Phase 1: User Entry Points

### 1.0A: New User Path (Signup)

**SRS Reference:** FR-AUTH-001

**Start State:** User lands on JobFits homepage

```
User clicks "Sign Up" or social login button
    ↓
Display: Signup Page
├─ Headline: "Find Your Next Opportunity"
├─ Primary CTA: [Sign up with Google] [Sign up with LinkedIn]
├─ Secondary: "or [Continue with Email]"
└─ Link: "Already have an account? [Log in]"
    ↓
OPTION 1: OAuth (Google or LinkedIn)
├─ User clicks [Sign up with Google]
├─ Redirect to Google OAuth consent screen
├─ User grants permissions (email, profile)
├─ Redirect back to JobFits
├─ System creates account with:
│  ├─ Email (from OAuth provider)
│  ├─ Name (from OAuth provider, editable)
...
│  ├─ Password strength: "Strong ✓" (real-time indicator)
│  └─ Terms: Checked ✓
├─ [Create Account] button
└─ On success: Redirect to Email Verification page
    ↓
Email Verification (both OAuth and Email/Password):
├─ Display: "Verify Your Email"
├─ Message: "We've sent a verification link to john@example.com"
├─ Status: "Link expires in 24 hours"
├─ Actions:
│  ├─ [Open email app] - Quick link to Gmail/Outlook/Apple Mail
│  ├─ [Resend verification email] - Limited to 3 attempts per hour
│  └─ [Change email] - If wrong email provided
├─ Progress indicator: (Step 1 of 3) "Verify Email"
└─ Timer showing countdown: "Expires in: 23h 45m"
    ↓
User clicks verification link in email:
├─ Email token validated (24-hour expiry)
├─ Email marked as verified in database
├─ Display: "✓ Email verified!"
├─ Message: "Redirecting to resume upload..."
└─ Auto-redirect after 2 seconds to Onboarding Phase (Flow 1)
    ↓
Email Verification Edge Cases:

If user clicks resend:
├─ Check: How many resends in past hour? (max 3)
├─ If <3: Send new verification email
│  └─ Display: "Verification link sent! Check your inbox."
├─ If ≥3: Show error
│  └─ "You've requested too many emails. Try again in 1 hour."

If user changes email:
├─ Require new email entry
├─ Send new verification to new email
├─ Old email address discarded
└─ New 24-hour timer starts

If verification link expires:
├─ Display: "Your verification link has expired"
├─ CTA: [Send new link]
└─ User can request another email (with rate limiting)

If user navigates away before verifying:
├─ Session stored (30-day JWT)
├─ Next login: Prompt to verify if not yet verified
├─ CTA: [Send verification email] on login screen
```

**Key Metrics:**
- Signup to verification: <2 minutes
- Email verification rate: >95%
- OAuth flow completion: <10 seconds
- Resend request rate: <10% of signups

**UX Notes:**
- Social login (OAuth) should be primary CTA – reduces friction by ~60% vs email signup
- Email verification should be auto-triggered; user shouldn't manually navigate
- Error messages must be specific ("Try a different Google account" not "Login failed")
- Show progress throughout signup (Step 1 of 3, Step 2 of 3, etc.)

---

### 1.0B: Returning User Path (Login)
...
│  └─ Rate limiting: Max 10 failed attempts per IP/hour
│     (After 10 failures: Lock for 15 minutes)
    ↓
...
│  └─ Display: "Too many login attempts. Try again in 15 minutes."
...
├─ User activity logged (for security audit)
├─ Redirect to:
│  ├─ Dashboard (if first time logging in) OR
│  ├─ Last page visited (if returning user) OR
│  ├─ Applications (if user has pending applications)
└─ Display: "Welcome back, John!"
```

**Key Metrics:**
- Login completion: <30 seconds
- Session timeout: 30 days or explicit logout
- Refresh token rotation: Automatic on each API call
- Failed login rate: <2%

**UX Notes:**
- OAuth login should be offered as equal option
- "Remember me" is optional (30-day sessions reduce need)
- "Forgot password?" link must be prominent
- Show security indicators (e.g., "This is a secure connection")
...
### 1.0C: Forgot Password Flow

**SRS Reference:** FR-AUTH-001 - "Password reset via email with token expiration (24 hours)"

**Critical Issue Fix:** This flow was completely missing from original user flows.

```
User clicks "Forgot password?" on login page OR settings page
    ↓
Display: Recover Your Password
├─ Headline: "Reset Your Password"
├─ Message: "Enter your email and we'll send a password reset link"
...
│  │  └─ Display success message (regardless of email existence)
│  └─ IF email NOT in system:
│     └─ Display success message (for security – don't reveal if email exists)
    ↓
Display success message:
├─ "Check your email for password reset link"
├─ Message: "We've sent a reset link to john@example.com"
├─ Countdown: "Link expires in 24 hours"
├─ Actions:
│  ├─ [Open email app] - Quick links to Gmail, Outlook, Apple Mail
│  ├─ [Resend reset link] - Limited to 1 attempt per 5 minutes
│  ├─ [Change email] - Re-enter different email
│  └─ [Back to login]
└─ Auto-suggest: "Check spam/junk folder if not in inbox"
    ↓
User receives password reset email:
├─ Subject: "Reset your JobFits password"
├─ From: noreply@jobfits.com
├─ Body includes:
│  ├─ Greeting: "Hi John,"
│  ├─ Message: "You requested to reset your password"
│  ├─ CTA button: [Reset Password] (links to reset URL)
│  ├─ Alternative link: Raw URL for email clients that don't support buttons
│  ├─ Expiration: "This link expires in 24 hours"
│  ├─ Security note: "If you didn't request this, ignore this email"
│  └─ No-reply footer: "Do not reply to this email"
└─ Delivery target: <1 minute (SRS requirement)
    ↓
User clicks reset link in email:
├─ Redirect to: /auth/reset-password?token=abc123xyz
├─ Backend validates:
│  ├─ Token exists in database
│  ├─ Token not expired (created <24 hours ago)
│  ├─ Token matches user email
│  └─ Token hasn't been used (one-time use)
    ↓
IF token valid:
├─ Display: Reset Password Form
├─ Fields:
│  ├─ New password [required, 8+ chars, uppercase, lowercase, number, special]
│  ├─ Confirm password [required, must match]
...
│  ├─ Invalidate all existing sessions (user must login again)
│  ├─ Log password change event (audit trail)
│  └─ Send confirmation email: "Your password was changed"
    ↓
Display: Success Message
├─ Headline: "Password Reset Successfully"
├─ Message: "Your password has been updated"
├─ Additional message: "All sessions have been logged out for security. Please log in again."
├─ [Log In] button
...
IF token is expired:
├─ Display: "Password reset link has expired"
├─ Message: "Links expire after 24 hours"
├─ CTA: [Request new link]
└─ Redirect to forgot password form

IF token is invalid/tampered:
├─ Display: "Invalid password reset link"
├─ Message: "This link may be broken or already used"
├─ CTA: [Request new link]
└─ For security: Log this attempt
...
IF user clicks same reset link twice:
├─ First click: Successful password reset
├─ Second click: "This link has already been used"
├─ CTA: [Request new link]
└─ For security: Log attempt
```

**Key Metrics:**
- Reset email delivery: <1 minute (SRS requirement)
- Link expiration: 24 hours (SRS requirement)
- Reset completion rate: >80% of users who request
- Time from reset request to new login: 5-10 minutes average

**UX Notes:**
- Don't reveal whether email exists in system (prevent account enumeration)
- Allow resend but with rate limiting (prevent abuse)
- Show security messaging throughout ("Your account is secure")
- After password reset, log user out of all sessions (security best practice)
